...
function MarketAnalysisComponent({ symbol, momentum, orderbook }: MarketAnalysisProps) {
  ...
  return (
    <div className="space-y-3">
      ...
      {momentum && (momentum.vwap !== undefined || momentum.rsi !== undefined || momentum.adx !== undefined || momentum.supertrend_direction !== undefined) && (
        <div className="border-t border-zinc-200 dark:border-zinc-700 pt-2">
          <div className="flex items-center gap-1 text-xs text-zinc-500 mb-2">
            Technical Indicators
            <InfoTooltip content="Real-time technical indicators calculated from Binance price data. Used by the strategy to confirm trade signals." />
          </div>
          <div className="grid grid-cols-4 gap-2 text-xs">
            {/* VWAP */}
            {momentum.vwap !== undefined && (
              <div className="bg-zinc-100 dark:bg-zinc-800/50 rounded p-2">
                <div className="flex items-center gap-1 text-zinc-500 mb-1">
                  VWAP
                  <InfoTooltip content="Volume Weighted Average Price. Price above VWAP = bullish, below = bearish." />
                </div>
                <div className={`font-mono font-medium ${
                  momentum.vwap_signal === "UP" ? "text-green-500" :
                  momentum.vwap_signal === "DOWN" ? "text-red-500" : "text-zinc-500"
                }`}>
                  {momentum.vwap_signal === "UP" ? "↑" : momentum.vwap_signal === "DOWN" ? "↓" : "—"}
                </div>
              </div>
            )}
            ...
            {momentum.rsi !== undefined && (
              <div className="bg-zinc-100 dark:bg-zinc-800/50 rounded p-2">
                <div className="flex items-center gap-1 text-zinc-500 mb-1">
                  RSI
                  <InfoTooltip content="Relative Strength Index (0-100). Below 30 = oversold (bullish), above 70 = overbought (bearish)." />
                </div>
                <div className={`font-mono font-medium ${
                  momentum.rsi < 30 ? "text-green-500" :
                  momentum.rsi > 70 ? "text-red-500" : "text-zinc-500"
                }`}>
            ...
            {momentum.adx !== undefined && (
              <div className="bg-zinc-100 dark:bg-zinc-800/50 rounded p-2">
                <div className="flex items-center gap-1 text-zinc-500 mb-1">
                  ADX
                  <InfoTooltip content="Average Directional Index. Above 25 = strong trend. Shows trend strength, not direction." />
                </div>
                <div className={`font-mono font-medium ${
                  momentum.adx >= 25 ? "text-purple-500" : "text-zinc-500"
                }`}>
            ...
            {/* Supertrend */}
            {momentum.supertrend_direction !== undefined && (
              <div className="bg-zinc-100 dark:bg-zinc-800/50 rounded p-2">
                <div className="flex items-center gap-1 text-zinc-500 mb-1">
                  STrend
                  <InfoTooltip content="Supertrend indicator. Clear UP/DOWN signal based on ATR volatility bands." />
                </div>
                <div className={`font-mono font-medium ${
                  momentum.supertrend_direction === "UP" ? "text-green-500" :
                  momentum.supertrend_direction === "DOWN" ? "text-red-500" : "text-zinc-500"
                }`}>
                  {momentum.supertrend_direction === "UP" ? "↑ UP" :
                   momentum.supertrend_direction === "DOWN" ? "↓ DN" : "—"}
                </div>
              </div>
            )}
          </div>
